...
import { StatesEntity } from "../../states/entities/StatesEntity";
...
import { AddressEntity } from "@modules/address/entities/AddressEntity";
...
export const UserController = {
    ...
    storeCustomer: async (request: Request, response: Response) => {
        ...
        try {
            ...
            const userCpf: string = stringFormatter.onlyNumberString(
                dataUser.cpf,
            );
            ...
            const getAddressCodePostal =
                await UserController.getCep(addressCodePostal);
            ...
            await dataSource.transaction(async (transactionalEntityManager) => {
                ...
                if (!companyPlatform) {
                    response.status(404).send({
                        message: "Plataforma da empresa não foi encontrada.",
                    });
                    return;
                }
    ...
    storeADM: async (request: Request, response: Response) => {
        ...
        try {
            ...
            const phoneNumber = stringFormatter.onlyNumberInt(
                address.phoneNumber,
            );
            ...
            await dataSource.transaction(async (transactionalEntityManager) => {
                ...
                const userTypeEntity =
                    transactionalEntityManager.getRepository(UserTypeEntity);
                const statesEntity =
        ...
        try {
            ...
            await dataSource.transaction(async (transactionalEntityManager) => {
                ...
                const platformEntity =
                    transactionalEntityManager.getRepository(PlatformEntity);
                const companyEntity =
                    transactionalEntityManager.getRepository(CompanyEntity);
                const userEntity =
                    transactionalEntityManager.getRepository(UserEntity);
                ...
                const userView = UserView.storeCompany(userStore, addressStore);
        ...
        try {
            await dataSource.transaction(async (transactionalEntityManager) => {
                ...
                if (isActive) {
                    ...
                    if (!platformEntity) {
                        response.status(404).send({
                            message: "Plataforma não encontrado.",
                        });
                        return;
                    }
                    ...
                    const moreThanMaxUsersPlan =
                        totalUsers.length > maxUsersPlan;
