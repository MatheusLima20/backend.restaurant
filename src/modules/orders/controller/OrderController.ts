    ...
    patch: async (request: Request, response: Response) => {
        ...
        try {
            dataSource.transaction(async (transactionalEntityManager) => {
                ...
                if (!oldOrder) {
                    response.send("Pedido não encontrado");
                    return;
                }
    ...
    patchSubtract: async (request: Request, response: Response) => {
        ...
        try {
            dataSource.transaction(async (transactionalEntityManager) => {
                ...
                if (!oldOrder) {
                    response.status(404).send({
                        message: "Erro ao pesquisar o pedido.",
                ...
                if (!product) {
                    response.status(404).send({
                        message: "Produto não encontrado",
                        error: "Não foi possivel pesquisar o produto.",
                    });
                    return;
                }
                ...
                if (!boxDay) {
                    response.status(404).send({
                        message: "Erro ao pesquisar o caixa.",
    ...
    patchChangeTableOrders: async (request: Request, response: Response) => {
        ...
        try {
            ...
            dataSource.transaction(async (transactionalEntityManager) => {
                ...
                if (!oldTable) {
                    response.status(404).send({
                        message: "Erro ao pesquisar a mesa.",
